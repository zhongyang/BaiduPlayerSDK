...
/**
 @abstract 结束当前视频的播放。
 @discussion 调用stop结束当前播放，如果需要重新播放该视频，需要调用[prepareToPlay]([CyberPlayerController prepareToPlay])方法。调用stop方法后，播放器开始进入关闭当前播放的操作，操作完成将发送CyberPlayerPlaybackDidFinishNotification通知。
 @since Available in CyberPlayerController 1.0 and later.
 */
- (void) stop;
/**
...
/**
 ...
 @discussion 当前版本view内只有视频帧图像，没有控制相关组件。相当于iOS系统播放器MPMoviePlayerController的控制方式controlStyle为MPMovieControlStyleNone的效果。
 @since Available in CyberPlayerController 1.0 and later.
 */
...
/**
 ...
 @discussion 当播放网络视频时，playableDuration值标记着已经加载视频的最大时刻（单位为秒），也就是说如果此刻断开网络，视频播放只能到playableDuration时刻。
 @since Available in CyberPlayerController 1.0 and later.
 */
